module.exports = function (grunt) {
	...
	var splitString = '<!--template_split-->';
	var firstSplit = apphtml.indexOf(splitString);
	var secondSplit = apphtml.indexOf(
		splitString,
		firstSplit + splitString.length
	);

	var htmlTop = apphtml.slice(0, firstSplit);
	var htmlBottom = apphtml.slice(secondSplit + splitString.length);
	...
	grunt.initConfig({
		...
		concat: {
			...
			dist: {
				options: {
					banner: htmlTop + '<style>\n',
					footer: '</script>' + htmlBottom,
					separator: '\n</style>\n\n<script>\n',
				},
				src: [
					'dist/build/Glyphr_Studio.min.css',
					'dist/build/Glyphr_Studio.min.js',
				],
			...
			core: {
				src: [
					'dev/js/_functions.js',
					'dev/js/io_glyphr_studio_project_import.js',
					'dev/js/obj_*.js',
				],
		...
		cssmin: {
			minimize: {
				src: ['dev/openSans.css', 'dev/fonts.css', 'dev/Glyphr_Studio.css'],
