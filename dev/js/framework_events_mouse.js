...
function editCanvasMouseEvent(ev) {
	...
	// debug(`ev.type: ${ev.type} - ev.button: ${ev.button}`);
	// debug(ev);
	mouseovercec();
	...
	if (ev.button === 0) {
		switch (_UI.selectedToolName) {
			case 'pathedit':
				eh.currentTool = eh.eh_pathedit;
				break;
			case 'shaperesize':
				eh.currentTool = eh.eh_shapeedit;
				break;
			case 'pan':
				eh.currentTool = eh.eh_pantool;
				break;
			case 'pathaddpoint':
				eh.currentTool = eh.eh_pathaddpoint;
				break;
			case 'newpath':
				eh.currentTool = eh.eh_addpath;
				break;
			case 'newrect':
				eh.currentTool = eh.eh_addrectoval;
				break;
			case 'newoval':
				eh.currentTool = eh.eh_addrectoval;
				break;
			case 'slice':
				eh.currentTool = eh.eh_slice;
				break;
			case 'kern':
				eh.currentTool = eh.eh_kern;
				break;
			case _UI.selectedToolName:
				eh.currentTool = eh.eh_pathedit;
		}
...
function Tool_ShapeEdit() {
	...
	this.mousedown = function (ev) {
		...
		if (eh.handle) {
			if (eh.handle === 'rotate') {
				...
				eh.rotationstartpoint.y =
					_UI.ms.shapes.getMaxes().ymax + _UI.rotatehandleheight / getView().dz;
	...
	this.mousemove = function (ev) {
		...
		if (this.dragging) {
			...
			var radians = calculateAngle(
				{ x: cx_sx(eh.mousex), y: cy_sy(eh.mousey) },
				eh.rotationcenter
			);
			var deltaRad = (radians -= Math.PI / 2);
			// debug(`\t rotation affordance radians rotation: ${radians}`);

			_UI.ms.shapes.rotationPreview(
				deltaRad,
				eh.rotationcenter,
				eh.isShiftDown
			);
	...
	this.mousemove = function (ev) {
		if (_UI.eventhandlers.tempnewbasicshape) {
			_UI.eventhandlers.tempnewbasicshape.xmax = Math.max(
				...
				cx_sx(_UI.eventhandlers.mousex)
			);
			...
			_UI.eventhandlers.tempnewbasicshape.ymax = Math.max(
				...
				cy_sy(_UI.eventhandlers.mousey)
			);
...
function Tool_NewPath() {
	...
	this.mousedown = function (ev) {
		...
		if (this.firstpoint) {
			...
			if (
				this.newshape.path.isOverFirstPoint(cx_sx(eh.mousex), cy_sy(eh.mousey))
			) {
	...
	this.mousemove = function (ev) {
		...
		if (this.dragging) {
			...
			if (
				Math.abs(this.currpt.P.x - cx_sx(eh.mousex)) >
					_GP.projectsettings.pointsize * 2 ||
				Math.abs(this.currpt.P.y - cy_sy(eh.mousey)) >
					_GP.projectsettings.pointsize * 2
			) {
...
function Tool_PathEdit() {
	...
	this.mousedown = function (ev) {
		...
		if (this.controlpoint) {
			...
			if (this.controlpoint.type === 'P') {
				...
				else if (!_UI.ms.points.isSelected(this.controlpoint.point))
					_UI.ms.points.select(this.controlpoint.point);
				setCursor('penSquare');
			} else {
	...
	this.mousemove = function (ev) {
		...
		var cp = _UI.ms.shapes.isOverControlPoint(
			cx_sx(eh.mousex),
			cy_sy(eh.mousey)
		);
...
function Tool_PathAddPoint() {
	...
	this.mousedown = function (ev) {
		...
		if (
			...
			var p = singleshape.path.insertPathPoint(
				this.addpoint.split,
				this.addpoint.point
			);
	...
	this.mousemove = function (ev) {
		var singleshape = _UI.ms.shapes.getSingleton();
		if (singleshape) {
			var pt = singleshape.path.getClosestPointOnCurve({
				x: cx_sx(_UI.eventhandlers.mousex),
				y: cy_sy(_UI.eventhandlers.mousey),
			});
