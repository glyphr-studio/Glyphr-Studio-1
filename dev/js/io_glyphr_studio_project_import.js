	...
	if (projvn.major === 1) {
		...
		if (projvn.minor < 13 || (projvn.minor === 13 && projvn.patch < 2)) {
			...
			for (var g in fcontent.glyphs) {
				if (fcontent.glyphs.hasOwnProperty(g))
					invertComponentInstanceRotation(fcontent.glyphs[g]);
			}

			for (var l in fcontent.ligatures) {
				if (fcontent.ligatures.hasOwnProperty(l))
					invertComponentInstanceRotation(fcontent.ligatures[l]);
			}

			for (var c in fcontent.components) {
				if (fcontent.components.hasOwnProperty(c))
					invertComponentInstanceRotation(fcontent.components[c]);
			}
		}
	}
...
function migrate_0_5_to_1_0(fc) {
	...
	fc.projectsettings.glyphrange = clone(
		fc.projectsettings.charrange,
		'migrate_0_5_to_1_0'
	);
	delete fc.fontchars;
...
function charToGlyph(gl) {
	...
	for (var s = 0; s < gshapes.length; s++) {
		...
		if (sh.objtype === 'linkedshapeinstance') {
			...
			gshapes[s] = new ComponentInstance({
				name: sh.name,
				link: sh.link,
				translatex: dx,
				translatey: dy,
...
function hydrateGlyphrProject(data, callback) {
	...
	if (data.projectsettings) {
		...
		ps.glyphrange.custom = data.projectsettings.glyphrange.custom || [];
	}
	...
	for (var a in template) {
		if (template.hasOwnProperty(a)) {
			if (typeof template[a] === 'object') {
				...
				if (importing.hasOwnProperty(a)) {
					if (typeof importing[a] === 'string' && trim)
						template[a] = removeEmptyStringInputs(importing[a]);
...
function finalizeGlyphrProject() {
	...
	ps.guides.capheight =
		...
		new Guide({
			name: 'capheight',
			type: 'horizontal',
			location: ps.capheight,
			editable: false,
			color: ps.colors.guide_light,
		});
	ps.guides.xheight =
		ps.guides.xheight ||
		new Guide({
	...
	if (!sp.isautowide && sp.glyphwidth === 0)
		sp.glyphwidth = Math.round(_GP.projectsettings.upm / 3);
