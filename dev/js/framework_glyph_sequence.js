...
**/
	...
	function GlyphSequence(oa){
		...
		this.maxes = oa.maxes || {};
		this.maxes.xmin = oa.maxes.xmin || 0;
		this.maxes.xmax = oa.maxes.xmax || Infinity;
		this.maxes.ymin = oa.maxes.ymin || 0;
		this.maxes.ymax = oa.maxes.ymax || Infinity;

		this.scale = oa.scale || 1;
		this.glyphstring = oa.glyphstring || '';
		this.textblocks = this.glyphstring.split('\n');
		this.linegap = oa.linegap || Math.round(_UI.projectsettings.upm / 4);
	...
	GlyphSequence.prototype.generateData = function() {
		...
		var curry = ((this.maxes.ymin + yoffset) / this.scale);
		var area = {
		...
		debug(`\t currs ${currx}, ${curry}, ${this.scale}`);
	...
	function calcY(starty, linenum, linegap) {
		var ps = _GP.projectsettings;
		return starty + ps.ascent + (linenum*((linegap + ps.upm)));
	}
