...
function loadPage_testdrive() {
	...
	var content =
		'<h1 class="pagetitle">Test Drive</h1><div class="pagecontent">' +
		'<textarea id="tdtextarea" onkeyup="_UI.testdrive.sampletext=this.value; redraw_TestDrive()">' +
		_UI.testdrive.sampletext +
		'</textarea><br>' +
...
function redraw_TestDrive() {
	...
	var tdHeight = Math.max(
		580,
		lastchar.view ? lastchar.view.dy * lastchar.view.dz : 0
	);
...
function drawTestDriveGlyphExtras(chardata) {
	...
	if (_UI.testdrive.showglyphextras) {
		...
		var drawy =
			(chardata.view.dy - _GP.projectsettings.ascent) * chardata.view.dz;
		...
		if (glyph) {
			if (flattenglyphs) {
				if (!_UI.testdrive.cache.hasOwnProperty(chardata.char)) {
					_UI.testdrive.cache[chardata.char] = new Glyph(
						clone(glyph, 'drawTestDriveGlyph')
...
function drawSampletextButtons() {
	...
	content += makeTDButton(
		'quick enemy movement will<br>jeopardize six of the gunboats'
	);
...
function makeTDButton(text) {
	...
	return (
		'<button class="sampletext" onclick="_UI.testdrive.sampletext=\'' +
		val +
		'\';redraw_TestDrive();">' +
		text +
		'</button><br>'
...
function drawTDOptions() {
	if (!_UI.testdrive.linegap)
		_UI.testdrive.linegap = _GP.projectsettings.linegap;
	if (!isval(_UI.testdrive.padsize))
		_UI.testdrive.padsize = _GP.projectsettings.defaultlsb;
	...
	content +=
		...
		_UI.testdrive.fontsize +
		'" onchange="changefontscale(this.value); redraw_TestDrive();"></td></tr>';
	content +=
		'<tr><td> 96dpi font size <span class="unit">(pt)</span> </td><td><input type="number" disabled="disabled" id="roughptsize" valu="75"/></td></tr>';
	content +=
		'<tr><td> line gap <span class="unit">(em units)</span> </td><td><input type="number" value="' +
		_UI.testdrive.linegap +
	...
	content +=
		...
		' </td><td>' +
		checkUI('_UI.testdrive.flattenglyphs', _UI.testdrive.flattenglyphs, false) +
		'</td></tr>';

	content +=
		'<tr><td colspan=2><button onclick="createimg();">generate png file</button></td></tr>';
	content += '</table>';
	return content;
}
