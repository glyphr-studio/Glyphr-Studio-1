...
function popOut() {
	...
	// Second Screen
	_UI.popout.document.head.appendChild(
		document.styleSheets[0].ownerNode.cloneNode(true)
	);
...
function make_NavPanels_PopOut() {
	...
	var onkern = _UI.current_page === 'kerning';
	var evmove =
		_UI.eventhandlers.currentTool.dragging ||
		_UI.eventhandlers.currentTool.resizing;

	document.getElementById('popout_pagenav').innerHTML = makePanel_PageNav();

	if ((onge || onlig || oncom) && !evmove)
		document.getElementById('popout_glyphchooser').innerHTML =
			makePanel_GlyphChooser();

	document.getElementById('popout_history').innerHTML = makePanel_History();

	if (!onkern)
		document.getElementById('popout_layerchooser').innerHTML =
	...
	if (ontd)
		document.getElementById('popout_attributes').innerHTML =
			makePanel_TestDriveAttributes();
	else if (onkern)
		document.getElementById('popout_attributes').innerHTML =
			makePanel_KerningAttributes();
	else
		document.getElementById('popout_attributes').innerHTML =
...
function make_NavPanels_PopIn() {
	...
	switch (_UI.current_panel) {
		...
		case 'npAttributes':
			...
			switch (_UI.current_page) {
				...
				case 'ligatures':
					...
					break;
				case 'kerning':
					np.innerHTML = makePanel_KerningAttributes();
					break;
				case 'test drive':
					np.innerHTML = makePanel_TestDriveAttributes();
					break;
			}
			break;
		...
		case 'npNav':
			...
			break;
		case 'npLayers':
			np.innerHTML = makePanel_LayerChooser();
			break;
		case 'npGuides':
			np.innerHTML = makePanel_Guides();
			break;
		case 'npHistory':
			np.innerHTML = makePanel_History();
			break;
	}
...
function loadPageContent() {
	switch (_UI.current_page) {
		case 'openproject':
			...
			break;
		case 'global actions':
			loadPage_globalactions();
			break;
		case 'font settings':
			loadPage_fontsettings();
			break;
		case 'project settings':
			loadPage_projectsettings();
			break;
		case 'export font':
			loadPage_exportfont();
			break;
...
function makePanel_NavTabs() {
	...
	if (_UI.current_panel === 'npNav') {
		...
		document.getElementById('navarea_tabs').style.backgroundColor =
			_UI.colors.gray.l90;
	} else {
		document.getElementById('navarea_tabs').style.backgroundColor =
			_UI.colors.gray.l80;
	...
	if (_UI.current_panel === 'npNav') {
		...
		if (!onNoNavPage()) {
			newsub +=
				'<button class="primarynavbutton" id="npNav" onclick="clickHamburger();">';
			newsub += makeIcon({
				name: 'button_npBack',
		...
		newsub +=
			'<div class="navarea_header" title="Navigate" style="background-color:' +
			_UI.colors.blue.l45 +
			';">';
		newsub +=
			'<button class="primarynavbutton" id="npNav" onclick="clickHamburger();">';
	...
	for (var i = 1; i < navarr.length; i++) {
		...
		newsub +=
			'<div class="np_section" title="' +
			navarr[i].substr(2) +
			'"><button class="primarynavbutton" id="' +
			navarr[i] +
			'" onclick="navigate({panel:\'' +
			navarr[i] +
			'\'});">';
		newsub += makeIcon({
			name: 'button_' + navarr[i],
			color: nfill,
			hovercolor: nhover,
		});
		newsub += '</button></div>';
	...
	// Bottom Left
	newsub +=
		'<div style="position:absolute; bottom:15px; left:0px; width:70px; text-align:center; cursor:pointer;">';

	// Contribute!
	newsub +=
		'<a href="#" style="color:' +
		_UI.colors.blue.l55 +
		'; font-size:18px;" onclick="openDialog(make_ContributeHTML());">give<br>back!</a>';
	newsub += '</div>';
...
function makePanel_PageNav() {
	...
	var newsub =
		'<div class="navarea_header" style="padding:12px 10px 8px 10px;">' +
		makeGlyphrStudioLogo({ fill: 'white', width: 150 }) +
		'</div>';
	...
	for (var i = 0; i < navarr.length; i++) {
		...
		if (navarr[i] === '_') {
			...
			newsub +=
				'<div style="cursor:pointer; background-color:rgb(0,113,170); height:32px; position:relative; left:-11px; top:-4px; padding:7px 0px 0px 14px;" title="one screen mode" onclick="popIn();">' +
				makeIcon({
					name: 'tool_popIn',
					color: 'white',
					hovercolor: 'white',
			...
			newsub +=
				'<a href="mailto:mail@glyphrstudio.com?subject=Hi%20Glyphr%20Studio&amp;body=' +
				genEmailContent() +
