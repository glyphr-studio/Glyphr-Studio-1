...
function SatChooser(pv) {
	...
	this.clickCallback =
		...
		function (c) {
			console.log(
				'Click callback function returned object: ' + JSON.stringify(c)
			);
		};
	...
	this.width = pv.width
		? this.getDiscreetWidth(pv.width)
		: this.getDiscreetWidth(200);
	...
	this.can.onclick = function (e) {
		...
		var ipx =
			Math.floor(e.offsetX - 1) * 4 +
			Math.floor(e.offsetY - 1) * that.imgData.width * 4;
