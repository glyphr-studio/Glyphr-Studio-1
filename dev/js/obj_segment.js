...
Segment.prototype.pointIsWithinMaxes = function (co) {
	...
	var re = co.x <= m.xmax && co.x >= m.xmin && co.y <= m.ymax && co.y >= m.ymin;
...
function findOverlappingLineSegmentIntersections(s1, s2) {
	...
	if (s1.containsPointOnLine(s2.getCoord(1)))
		re.push('' + s2.p1x + '/' + s2.p1y);
	if (s1.containsPointOnLine(s2.getCoord(4)))
		re.push('' + s2.p4x + '/' + s2.p4y);

	if (s2.containsPointOnLine(s1.getCoord(1)))
		re.push('' + s1.p1x + '/' + s1.p1y);
	if (s2.containsPointOnLine(s1.getCoord(4)))
...
Segment.prototype.isRedundantTo = function (s) {
	...
	return (
		s.containsPointOnLine(this.getCoord(1)) &&
		s.containsPointOnLine(this.getCoord(4))
	);
};
...
function segmentsAreEqual(s1, s2, threshold) {
	...
	if (
		coordsAreEqual(s1.getCoord(1), s2.getCoord(1), threshold) &&
		...
		if (s1.line && s2.line) {
			...
			coordsAreEqual(s1.getCoord(2), s2.getCoord(2), threshold) &&
...
Segment.prototype.isLine = function (precision) {
	...
	var red =
		pointsAreCollinear(this.getCoord(1), this.getCoord(4), this.getCoord(2)) &&
