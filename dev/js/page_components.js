...
function createNewComponent(pglyph) {
	...
	pglyph =
		pglyph ||
		new Glyph({ name: 'Component ' + (getLength(_GP.components) + 1) });
...
function deleteComponentConfirm() {
	...
	if (uia.length > 0) {
		content +=
			'This Component is linked to the following Glyphs as a Component Instance:<br><ul>';
	...
	content +=
		'<br><br><button class="buttonsel" onclick="deleteComponent();">delete this component</button> &nbsp; <button onclick="closeDialog();">cancel</button>';
