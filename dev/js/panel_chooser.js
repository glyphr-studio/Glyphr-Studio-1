...
function makePanel_GlyphChooser() {
	...
	if (_UI.current_page === 'glyph edit') {
		...
		if (emptyligs)
			content +=
				'<b>Ligatures</b> are when a sequence of two or more glyphs are replaced with a special combination glyph. ' +
		...
		if (emptycoms)
			content +=
				'<b>Components</b> are glyphs that you can re-use in other glyphs. ' +
		...
		content +=
			"<button onclick=\"createNewComponent();history_put('Create New Component');navigate({panel:'npAttributes'});\">create new component</button><br>";
		if (!emptycoms)
			content +=
...
function make_GlyphChooser(gcdata) {
	...
	if (
		...
		(_UI.current_page === 'import svg' &&
			(pluralGlyphRange() ||
				getLength(_GP.components) ||
				getLength(_GP.ligatures)))
	) {
...
function toggle_GlyphChooser() {
	...
	if (isBigDialogOpen()) {
		document.getElementById('bigDialogScrollContent').innerHTML =
			make_GlyphChooser(_UI.glyphchooser.dialog);
	} else {
		...
		if (!isNaN(parseInt(selrange))) {
			...
			switch (selrange) {
				...
				case 'latinextendedb':
					...
					break;
				case 'components':
					selectGlyph(getFirstID(_GP.components), true);
					break;
				case 'ligatures':
					selectGlyph(getFirstID(_GP.ligatures), true);
					break;
			}
		}
...
function make_GlyphChooser_DropDown(ch) {
	...
	if (ch === 'glyphs' || ch === 'all') {
		...
		content +=
			'<br><span class="textaction" onclick="showGlyphRangeChooser();">Add additional glyph ranges</span><br><br>';
	}
	...
	if (ch === 'components' || ch === 'all') {
		if (getLength(_GP.components)) {
			...
			content +=
				'<span class="units">(' + getLength(_GP.components) + ')</span>';
			content += '</button>';
		}
	}
	...
	if (ch === 'ligatures' || ch === 'all') {
		if (getLength(_GP.ligatures)) {
			...
			content +=
				'<span class="units">(' + getLength(_GP.ligatures) + ')</span>';
			content += '</button>';
		}
	}
...
function make_GlyphChooser_Content(gcdata) {
	...
	var deletefooter =
		'</div><div class="panel_section"><button onclick="deleteSelectedGlyph()" style="display:block;">Delete selected glyph</button></div>';
	var footer = '</div>';
	...
	if (sel === 'latinsupplement') {
		...
		for (
			var s = _UI.glyphrange.latinsupplement.begin;
	...
	if (sel === 'latinextendeda') {
		...
		for (
			var a = _UI.glyphrange.latinextendeda.begin;
	...
	if (sel === 'latinextendedb') {
		...
		for (
			var b = _UI.glyphrange.latinextendedb.begin;
	...
	return (
		'<div class="panel_section"><h3 style="margin-top:0;">whoops!</h3>' +
	...
	if (success) {
		showToast(
			'Deleted glyph: ' +
				...
				'<br>' +
				selwi.name +
				"<br>Don't worry, you can undo this action.",
			2500
		);
