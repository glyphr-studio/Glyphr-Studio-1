...
function findPathIntersections(p1, p2, onlyfirst) {
	...
	intersects = intersects.concat(
		findPathPointBoundaryIntersections(p1, p2, onlyfirst)
	);
	if (intersects[0] && onlyfirst) return intersects[0];
...
Path.prototype.makeSVGpathData = function () {
	...
	re +=
		'M' +
		round(this.pathpoints[0].getPx(), roundvalue) +
		',' +
		round(this.pathpoints[0].getPy(), roundvalue);
	// debug('GENPATHPOSTSCRIPT:\n\t ' + re);
...
Path.prototype.getClosestPointOnCurve = function (coord, wantsecond) {
	...
	for (var pp = 0; pp < this.pathpoints.length; pp++) {
		...
		for (var t = 0; t < 1; t += 1 / grains) {
			...
			if (d < mindistance) {
				if (first && first.point !== pp)
					second = clone(first, 'Path.getClosestPointOnCurve');
				mindistance = d;
				first = {
					point: pp,
					split: t,
