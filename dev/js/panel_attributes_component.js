...
//	---------------------------------
	...
	function componentInstanceDetails(s){
		...
		content += '<tr>'+
			...
			'<td>'+
				'<div class="lockwrapper">'+
					lockUI('_UI.mss.getSingleton().xlock', s.xlock, 'xlock')+
				...
				dimSplit() +
				'<div class="lockwrapper">'+
					lockUI('_UI.mss.getSingleton().ylock', s.ylock, 'ylock')+
		...
		content += '<tr>'+
			...
			'<td>'+
				'<div class="lockwrapper">'+
					lockUI('_UI.mss.getSingleton().wlock', s.wlock, 'wlock')+
				...
				dimSplit()+
				'<div class="lockwrapper">'+
					lockUI('_UI.mss.getSingleton().hlock', s.hlock, 'hlock')+
		...
		content += '<tr>'+
			...
			'<td>' + checkUI('_UI.mss.getSingleton().flipew', s.flipew, true) + '</td>'+
		'</tr>';

		content += '<tr>'+
			'<td> flip vertical </td>'+
			'<td>' + checkUI('_UI.mss.getSingleton().flipns', s.flipns, true) + '</td>'+
		'</tr>';

		content += '<tr>'+
			'<td> reverse winding </td>'+
			'<td>' + checkUI('_UI.mss.getSingleton().reversewinding', s.reversewinding, true) + '</td>'+
		'</tr>';
	...
	function updateComponentInstanceDetail(key, value, id) {
		var oldval = _UI.mss[key];
		_UI.mss[key] = value;
		var ts, w, h;

		if(_UI.mss.ratiolock){
			ts = _UI.mss.getTransformedGlyph();
			w = (ts.maxes.xmax - ts.maxes.xmin);
			h = (ts.maxes.ymax - ts.maxes.ymin);

			if(key === 'scalew'){
				_UI.mss.scaleh += ((value - oldval) * (h / w));
			} else if (key === 'scaleh'){
				_UI.mss.scalew += ((value - oldval) * (w / h));
			}
		}
