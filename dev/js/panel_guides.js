...
function makePanel_Guides() {
	...
	content +=
		...
		'<tr><td style="width:20px">' +
		checkUI('_UI.showguidelabels', _UI.showguidelabels, true) +
		'</td>' +
	...
	if (_UI.current_page !== 'kerning') {
		content +=
			'<br><h3 style=" margin-bottom:10px;">snapping</h3>' +
			'<table style="width:100%;">' +
			'<tr><td>' +
			checkUI(
				'_GP.projectsettings.snaptogrid',
				_GP.projectsettings.snaptogrid,
				true
			) +
			'</td>' +
			...
			'<tr><td>' +
			checkUI(
				'_GP.projectsettings.snaptoguides',
				_GP.projectsettings.snaptoguides,
				true
			) +
			'</td>' +
			'<td><label for="snaptoguides">snap to guides</label></td></tr>' +
			'<tr><td style="width:30px;"><input type="text" value="' +
			ps.snapdistance +
...
function makeOneGuideRow(guide, path, currviz, id) {
	...
	if (!sys) {
		re +=
			" onmouseover=\"hideAllSatChoosers(); this.style.cursor='pointer'; this.style.borderColor='" +
			guide.color +
			'\';"';
