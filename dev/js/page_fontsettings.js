...
function loadPage_fontsettings() {
	...
	content +=
		...
		ps.ascent +
		'" onchange="_GP.projectsettings.ascent = Math.abs(parseInt(this.value));"></td><td><span class="unit">(em units)</span></td></tr>' +
		'<tr><td>Cap height: </td><td><input type="number" value="' +
		ps.capheight +
		'" onchange="_GP.projectsettings.capheight = Math.abs(parseInt(this.value));"></td><td><span class="unit">(em units)</span></td></tr>' +
		'<tr><td>x Height: </td><td><input type="number" id="metric-xheight" value="' +
		ps.xheight +
		'" onchange="_GP.projectsettings.xheight = Math.abs(parseInt(this.value));"></td><td><span class="unit">(em units)</span></td></tr>' +
		'<tr><td>Descent height: </td><td><input type="number" id="metric-des" value="' +
		ps.descent +
		'" onchange="_GP.projectsettings.descent = Math.abs(parseInt(this.value))*-1;"/></td><td><span class="unit">(em units)</span></td></tr>' +
		'<tr><td><b>Total Units per Em: </b></td><td><input type="number" value="' +
		ps.upm +
		'" onchange="_GP.projectsettings.upm = Math.abs(parseInt(this.value));"/></td><td><span class="unit">(em units)</span></td></tr>' +
		'</table>' +
		'<div class="effect">It is highly recommended that Units per Em either be 1000, or a power of 2 (like 1024 or 2048). ' +
		'UPM values other than these run a risk of not working with some applications.</div><br>';
	...
	content +=
		...
		ps.defaultlsb +
		'" onchange="_GP.projectsettings.defaultlsb = Math.abs(parseInt(this.value)) || 0;"></td><td><span class="unit">(em units)</span></td></tr>' +
		'<tr><td>Right Side Bearing: </td><td><input type="number" value="' +
		ps.defaultrsb +
		'" onchange="_GP.projectsettings.defaultrsb = Math.abs(parseInt(this.value)) || 0;"></td><td><span class="unit">(em units)</span></td></tr>' +
		'</table>';
	...
	content +=
		...
		'<td class="uicolumn">' +
		checkUI(
			'_GP.projectsettings.glyphrange.basiclatin',
			ps.glyphrange.basiclatin,
			false,
			false,
			'activeRangesChanged'
		) +
	...
	for (var t = 0; t < bl.length; t++) {
		content += makeRangePreviewGlyph(bl[t] * 1);
	}
	...
	content +=
		...
		'<td class="uicolumn">' +
		checkUI(
			'_GP.projectsettings.glyphrange.latinsupplement',
			ps.glyphrange.latinsupplement,
			false,
			false,
			'activeRangesChanged'
		) +
	...
	content +=
		...
		'<td class="uicolumn">' +
		checkUI(
			'_GP.projectsettings.glyphrange.latinextendeda',
			ps.glyphrange.latinextendeda,
			false,
			false,
			'activeRangesChanged'
		) +
	...
	content +=
		...
		'<td class="uicolumn">' +
		checkUI(
			'_GP.projectsettings.glyphrange.latinextendedb',
			ps.glyphrange.latinextendedb,
			false,
			false,
			'activeRangesChanged'
		) +
	...
	content +=
		...
		'<button onclick="addCustomGlyphRange();">Add Range</button>' +
		...
		if (meta.hasOwnProperty(m) && m !== 'font_family') {
			if (meta[m] === '{{sectionbreak}}') {
				...
				if (m === 'shared') {
					...
					content +=
						'These properties are shared between OTF and SVG font file formats.';
				} else if (m === 'otf') {
					content += '<h2>OTF</h2>';
					content +=
						'These properties will be saved with Open Type files when they are exported.';
				} else if (m === 'svg') {
					content += '<h2>SVG</h2>';
					content +=
						"These properties are based on the CSS @font-face standard.  More information can be found at the W3C's <a href='https://www.w3.org/TR/CSS2/fonts.html' target='_blank'>Fonts Page</a> and their <a href='https://www.w3.org/TR/2008/REC-CSS2-20080411/fonts.html#select' target='_blank'>CSS @font-face Page</a>.";
				}
				content += '</p></td></tr>';
			} else {
				meta[m] = meta[m] || '""';
				content += '<tr>';
				content +=
					'<td class="propname" style="padding-top:2px;">' +
					m.replace(/_/g, '-') +
					'</td>';
				content +=
					'<td><input type="text" value="' +
					escapeHTMLValues(meta[m]) +
					'" onchange="_GP.metadata.' +
					m +
					' = removeEmptyStringInputs(this.value);"/></td>';
				content +=
					'<td class="prophelp" style="padding-top:2px;">' +
					_UI.metadatahelp[m] +
					'</td>';
				content += '</tr>';
			}
...
function addCustomGlyphRange(newrange) {
	...
	if (newrange) {
		...
		for (var r = 0; r < ranges.custom.length; r++) {
			if (
				ranges.custom[r].begin === newrange.begin &&
		...
		if (
			!(
				ranges.basiclatin ||
				ranges.latinextendeda ||
				ranges.latinextendedb ||
				ranges.latinsupplement
			)
		)
			activeRangesChanged();
		...
		if (document.getElementById('glyphchooser'))
			redraw({ calledby: 'addCustomGlyphRange', redrawcanvas: false });
		if (document.getElementById('unicoderangepreviewarea'))
			previewGlyphRange(newrange);
	}
}
...
function getCustomRange(filterbasicrange, dontclearinputs) {
	...
	if (document.getElementById('customrangename'))
		newrange.name = escapeHTMLValues(
			document.getElementById('customrangename').value
		);
	newrange.begin = parseUnicodeInput(
		document.getElementById('customrangebegin').value
	)[0];
	newrange.end = parseUnicodeInput(
	...
	// maxes
	if (filterbasicrange) {
		newrange.begin = Math.max(
			newrange.begin,
			_UI.glyphrange.latinextendedb.end + 1
		);
		newrange.end = Math.max(
			newrange.end,
			_UI.glyphrange.latinextendedb.end + 2
		);
	}
	...
	newrange.name =
		newrange.name ||
		'Glyph Range ' + (_GP.projectsettings.glyphrange.custom.length + 1);
...
function updateCustomRangeTable() {
	...
	if (cr.length) {
		for (var c = 0; c < cr.length; c++) {
			...
			content +=
				'<span class="textaction" onclick="removeCustomGlyphRange(' +
				c +
				');">remove</span>';
			content += '&emsp;';
			content +=
				'<span class="textaction" onclick="editCustomGlyphRange(' +
				c +
				');">edit</span>';
			content += '</td></tr>';
		}
		...
		content +=
			'<tr><td colspan="4"><i style="opacity: 0.5">No custom ranges have been added</i></td></tr>';
	}
	content += '</table><br>';
...
function showGlyphRangeChooser() {
	...
	content +=
		'<div id="unicoderangepreviewarea"><h2>preview</h2><div class="glyphrangepreview">';
	content += 'Select glyph range from the right to preview it here.<br><br>';
	content +=
		'These are only advanced glyph ranges, standard Latin ranges can be added or removed from Font Settings.</div></div>';
	...
	for (var b = 4; b < _UI.unicodeBlocks.length; b++) {
		...
		chooserContent +=
			'<span class="textaction" onclick="previewGlyphRange({begin:' +
			decToHex(block.begin) +
			', end:' +
			decToHex(block.end) +
			", name:'" +
			block.name +
		...
		chooserContent += '&emsp;';
		chooserContent +=
			'<span class="textaction" onclick="addCustomGlyphRange({begin:\'' +
			decToHex(block.begin) +
			"', end:'" +
			decToHex(block.end) +
			"', name:'" +
...
function previewGlyphRange(range) {
	...
	content += '</div>';
	content +=
		'<button class="buttonsel" onclick="addCustomGlyphRange({begin:\'' +
		decToHex(range.begin) +
		"', end:'" +
		decToHex(range.end) +
		"', name:'" +
		range.name +
