	...
	if (scs.length > 0) {
		...
		for (var i = scs.length - 1; i >= 0; i--) {
			...
			if (ts.objtype === 'componentinstance') {
				content +=
					'<td class="layerthumb">' +
					ts.getTransformedGlyph().makeSVG() +
					'</td>';
...
function updateLayerActions() {
	...
	var shapeactions = '';
	shapeactions +=
		'<button title="Add Shape\nCreates a new default shape and adds it to this glyph" onclick="addShape(); history_put(\'Add Shape\'); redraw({calledby:\'updateactions\'});">' +
	...
	if (selshapes > 0)
		shapeactions +=
			'<button title="Delete\nRemoves the currently selected shape or shapes from this glyph" onclick="_UI.ms.shapes.deleteShapes(); history_put(\'Delete Shape\'); redraw({calledby:\'updateactions\'});">' +
