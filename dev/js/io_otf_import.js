	...
	function setupFontImport() {
		...
		if (font && font.glyphs && font.glyphs.length) {
			...
			if (font.glyphs.length < _UI.overflowcount || filter) {
				...
				document.getElementById('openprojecttableright').innerHTML =
					make_ImportFilter(font.glyphs.length, 0, 'ioOTF_importOTFfont');
			}
	...
	function importOneGlyph() {
		...
		if (uni === false || uni === '0x0000') {
			...
			if (getUnicodeName(uni) === '[name not found]')
				_GP.projectsettings.glyphrange.filternoncharpoints = false;
		...
		for (var c = 0; c < additionalGlyphs.length; c++) {
			...
			for (var b = 0; b < _UI.unicodeBlocks.length; b++) {
				...
				if (
					current >= block.begin &&
					current <= block.end &&
					addedBlocks.indexOf(block.begin) < 0
				) {
					addedBlocks.push(block.begin);
					ranges.push({
						begin: decToHex(block.begin),
						end: decToHex(block.end),
						name: block.name,
					});
				}
			}
