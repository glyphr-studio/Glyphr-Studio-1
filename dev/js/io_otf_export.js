...
function ioOTF_exportOTFfont() {
	...
	function getNextLigatureCodePoint() {
		...
		console.warn(
			'The entire Unicode Private Use Area (U+E000 to U+F8FF) seems to be taken. Ligatures will now be added to the block starting at U+AC00.'
		);
	...
	function populateExportLists() {
		...
		for (var c in _GP.glyphs) {
			if (_GP.glyphs.hasOwnProperty(c) && isGlyphInActiveRange(c, ranges)) {
				if (parseInt(c)) {
					tg = new Glyph(
						clone(_GP.glyphs[c], 'ioOTF export.populateExportLists')
					);
		...
		for (var l in _GP.ligatures) {
			if (_GP.ligatures.hasOwnProperty(l)) {
				if (l.indexOf('0x', 2) > -1) {
					tg = new Glyph(
						clone(_GP.ligatures[l], 'ioOTF export.populateExportLists')
					);
					...
					if (ligWithCodePoint) {
						...
						var licp = new Glyph(
							clone(
								_GP.ligatures[l],
								'ioOTF export.populateExportLists - ligature with code point'
							)
						);
					...
					console.warn(
						'Skipped exporting ligature ' +
							l +
							' - only has one source character'
					);
				}
			}
		}
	...
	function generateOneGlyph() {
		...
		if (currentExportNumber < exportGlyphs.length) {
			...
			if (
				comb &&
				glyph.shapes.length <= _GP.projectsettings.maxcombineshapesonexport
			)
	...
	function generateOneLigature() {
		...
		if (currentExportNumber < exportLigatures.length) {
			...
			if (
				comb &&
				liga.shapes.length <= _GP.projectsettings.maxcombineshapesonexport
			)
			...
			var indexList = subList.map(function (v) {
				return codePointGlyphIndexTable[charToHex(v)];
			});
	...
	if (exportGlyphs.length === 0 && exportLigatures.length === 0) {
		showToast(
			'No glyphs or ligatures found to export!<br>Go to Font Settings &gt; Glyph Ranges<br> to select ranges to export.',
			6000
		);
	} else {
		if (exportGlyphs.length === 0)
			showToast(
				'Warning: No glyphs selected for export!<br>Go to Font Settings &gt; Glyph Ranges<br> to select ranges to export.',
				6000
			);
...
function assembleActiveRanges() {
	...
	var ranges = clone(
		_GP.projectsettings.glyphrange.custom,
		'ioOTF export.assembleActiveRanges'
	);
	if (_GP.projectsettings.glyphrange.latinextendedb)
		ranges.unshift({
			begin: _UI.glyphrange.latinextendedb.begin,
			end: _UI.glyphrange.latinextendedb.end,
		});
	if (_GP.projectsettings.glyphrange.latinextendeda)
		ranges.unshift({
			begin: _UI.glyphrange.latinextendeda.begin,
			end: _UI.glyphrange.latinextendeda.end,
		});
	if (_GP.projectsettings.glyphrange.latinsupplement)
		ranges.unshift({
			begin: _UI.glyphrange.latinsupplement.begin,
			end: _UI.glyphrange.latinsupplement.end,
		});
	if (_GP.projectsettings.glyphrange.basiclatin)
		ranges.unshift({
			begin: _UI.glyphrange.basiclatin.begin,
			end: _UI.glyphrange.basiclatin.end,
		});
