...
function update_ToolsArea() {
	...
	zoom +=
		...
		'</button>';
	zoom +=
		'<button title="zoom: fit to screen" class="tool" onclick="autoCalculateView(); redraw({calledby:\'updatetools\'});">' +
	...
	donepath +=
		'<button class="buttonsel" style="width:94px; font-size:.8em; padding:2px;" title="done editing path" onclick="clickTool(\'pathedit\');">done editing path</button>';
	...
	ctxg += checkUI(
		'_GP.projectsettings.showcontextglyphguides',
		_GP.projectsettings.showcontextglyphguides,
		true
	);
	ctxg +=
	...
	ctxg += '</div>';
	ctxg +=
		'<input type="text" id="contextglyphsinput" oninput="updateContextGlyphData(); autoCalculateView(); redraw({calledby:\'updatetools\', redrawtools: false, redrawpanels: false});" ';
	...
	ctxg += '<br>';
	ctxg +=
		'<button class="tool" title="Previous glyph" onclick="selectPreviousGlyph();">' +
		makeToolButton({ name: 'tool_previousGlyph' }) +
		'</button>';
	ctxg +=
		'<button class="tool" title="Next glyph" onclick="selectNextGlyph();">' +
		makeToolButton({ name: 'tool_nextGlyph' }) +
		'</button>';
	ctxg += '</div>';
	...
	getEditDocument().getElementById('toolsarea_upperleft').innerHTML =
		toolcontent;
	getEditDocument().getElementById('toolsarea_upperright').innerHTML =
		viewcontent;
	getEditDocument().getElementById('toolsarea_lowerleft').innerHTML =
		utilitiescontent;
...
function mouseovercec() {
	...
	updateCursor();
	if (_UI.hamburger.state !== 0 && _UI.current_panel !== 'npNav')
		goHamburger(false);
...
function mouseoutcec() {
	...
	updateCursor();
	if (_UI.hamburger.state !== 11 && _UI.current_panel !== 'npNav')
		goHamburger(true);
...
function showCtxGlyphsOptions() {
	getEditDocument().getElementById('contextglyphsoptions').style.display =
		'block';
	getEditDocument().getElementById('contextglyphsoptionsbutton').onclick =
		hideCtxGlyphsOptions;
...
function makeKeyboardShortcutsTable() {
	return `<table style='margin:20px 40px 40px 0px;'>
		...
		<tr><td>
			...
			<tr><td class='keycol'><span class='keycallout'>mouse wheel click</span><br>or <span class='keycallout'>spacebar</span></td><td>pan the edit canvas</td></tr>
...
function drawContextGlyphLeftLineExtras(char, seq) {
	var alpha = transparencyToAlpha(
		_GP.projectsettings.colors.systemguidetransparency
	);
	var color = RGBAtoRGB('rgb(204,81,0)', alpha);
	drawVerticalLine(char.view.dx * char.view.dz, false, color);

	var kern = calculateKernOffset(
...
function drawGlyphKernExtra(kern, rightx, topy, scale) {
	...
	var color = RGBAtoRGB(
		'rgb(255,0,255)',
		transparencyToAlpha(_GP.projectsettings.colors.systemguidetransparency)
	);
...
function drawContextGlyph(char) {
	...
	char.glyph.drawGlyph(
		...
		transparencyToAlpha(_GP.projectsettings.colors.contextglyphtransparency),
		true
	);
...
function autoCalculateView() {
	...
	if (_UI.current_page === 'kerning') {
		...
		leftwidth = getGlyphSequenceAdvanceWidth(
			_UI.contextglyphs.leftseq.glyphstring
		);
		rightwidth = getGlyphSequenceAdvanceWidth(
			_UI.contextglyphs.rightseq.glyphstring
		);
...
function getSelectedWorkItemID() {
	switch (_UI.current_page) {
		case 'glyph edit':
			return _UI.selectedglyph;
		case 'import svg':
			return _UI.selectedsvgimporttarget;
		case 'ligatures':
			return _UI.selectedligature;
		case 'components':
			return _UI.selectedcomponent;
		case 'kerning':
...
function draw_RotationAffordance(accent, thickness) {
	...
	thickness = thickness || 1;
	var center = clone(
		_UI.eventhandlers.rotationcenter,
...
function draw_CircleHandle(center) {
	...
	_UI.glypheditctx.arc(
		center.x,
		center.y,
		_GP.projectsettings.pointsize / 2,
		0,
		Math.PI * 2,
		true
	);
...
function getBoundingBoxHandleDimensions(maxes, thickness) {
	...
	dimensions.midx = Math.floor(
		sx_cx(maxes.xmin) + (sx_cx(maxes.xmax) - sx_cx(maxes.xmin)) / 2 - hp
	);
	...
	dimensions.midy = Math.floor(
		sy_cy(maxes.ymax) + (sy_cy(maxes.ymin) - sy_cy(maxes.ymax)) / 2 - hp
	);
...
function drawGuides() {
	...
	var ps = _GP.projectsettings;
	var onglyphedit =
		_UI.current_page === 'glyph edit' || _UI.current_page === 'ligatures';
	var onkern = _UI.current_page === 'kerning';
	// debug('\t ps.guides: ');
	// debug(ps.guides);
...
function setupEditCanvas() {
	...
	_UI.glypheditcanvas.addEventListener(
		'mousedown',
		editCanvasMouseEvent,
		false
	);
	_UI.glypheditcanvas.addEventListener(
		'mousemove',
